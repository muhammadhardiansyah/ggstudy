...
export const HomePageClient: React.FC<HomePageClientProps> = ({ initialMaterials }) => {
  const [materialsList, setMaterialsList] = useState<MaterialItem[]>(initialMaterials);
  ...
  // Refetch on mount with no-store to ensure latest DB status
  useEffect(() => {
  ...
  }, [materialsList, searchQuery, selectedLevel]);
